...
const productos = {
  300: "762", // 300 megas tiene id=762
  100: "763",
  50: "764",
};
export const Formulario = ({ velocidad }) => {
  ...
  // Seteamos el nacimiento desde un inicio para evitar error de entradas
   const hoy = new Date();
...
/*   useEffect(() => {
    inputs.nacimiento = mayorEdad.toISOString().substr(0, 10);
    setNacimiento(inputs.nacimiento);
  }, []);
 */
  ...
  const fetchData = async () => {
    ...
    /* const aux = nacimiento.split("-");
    const formatDate = `${aux[2]}/${aux[1]}/${aux[0]}`;
 */
